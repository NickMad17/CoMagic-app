...
export const AuthForm: React.FC<Props> = observer(({className}) => {
    ...
    const onFinish: FormProps<FieldType>["onFinish"] = (values) => {
        ...
        if (regMode && values.first_name && values.last_name) {
            registration(values.email, values.password, values.first_name, values.last_name).then(() => {
                if (User.error) {
                    error(User.error)
                } else {
    ...
    return (<>
            {contextHolder}
            <h2 className='text-center text-[25px] font-medium mt-[6vh]'>{regMode ? 'Sign Up' : 'Sign In'}</h2>
            {User.loading && <Loader/>}
            <Form
